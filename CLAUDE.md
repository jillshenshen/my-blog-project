...
- [x] **相簿系統**（`/albums`、`/albums/[slug]`、後台 `/admin/albums` CRUD）
  ...
  - 前台詳細頁：方形 grid + Client `AlbumLightbox`（← → / Esc / 點背景關閉、caption + 拍攝日期、鎖捲動；**手機隱藏左右箭頭**，改用滑動切換避免擋住照片）
  - 後台：基本資訊 form、多檔上傳、每張可編輯 alt / caption / taken_at、@dnd-kit 拖曳排序、設為封面、刪除（含 Storage 同步刪除）
...
- [x] **相關文章**：文章內頁底部 `RelatedPosts`（同分類取最新 3 篇，響應式 grid 1/2/3 cols；同分類無其他文章時隱藏）
- [ ] 按讚（評估後不做）
...
- [x] **GSC 驗證 meta tag**：`app/layout.tsx` 加 `metadata.verification.google`（commit `efce737`，已部署）
- [x] **內文 `<img>` lazy load**：`lib/utils/optimize-images.ts` 在 render 時對 Tiptap 輸出的 HTML `<img>` 加上 `loading="lazy" decoding="async"`，避免內文 9.5MB 圖片跟封面圖搶頻寬，行動裝置 LCP 從 7.1s → 5.6s、CLS 0.193 → 0、效能分數 67 → 74（commit `06ba931`）
  - 註：原本嘗試把 src 改寫成 `/_next/image` 走 Vercel 代理（下載量可省 9 成），但 Vercel 圖片優化是分地理區的 cache，Lighthouse 從美國測時永遠是冷快取、每張要 1-2s 的 transform，反而把 LCP 拉到 17s；故只保留 lazy 不重寫 src
- [ ] ~~Google Search Console 提交~~（暫緩：`*.vercel.app` 預設 URL 帶 `x-robots-tag: noindex` header，**Vercel 端無法移除**，GSC 驗證會被擋。等買自訂網域後再做）
- [ ] Lighthouse 進一步效能微調（LCP 受限於 Vercel free tier image API cold start + 慢速 4G 模擬下載時間，5.6s 已近物理下限；要再降需要架構級調整）
